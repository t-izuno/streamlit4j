...
/**
 * Port that stores download payloads keyed by an opaque token. Used by {@code St.downloadButton(byte[]...)} to expose
 * downloadable bytes to the frontend via a stable URL.
 */
public interface DownloadStore {
     ...
     *
     * @param filename
     *            suggested filename when the client saves the asset
     * @param contentType
     *            MIME type for the response
     * @param bytes
     *            raw payload to serve
     */
